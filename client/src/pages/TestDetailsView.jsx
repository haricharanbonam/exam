    ...
    <div className="min-h-screen bg-slate-50 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <button
          ...
          className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-slate-900 transition"
        >
          ← Back to Dashboard
        </button>

        <div className="bg-white rounded-2xl p-8 shadow-sm ring-1 ring-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900">{test.title}</h1>
            <p className="text-slate-500 mt-1">Exam Code: <span className="font-mono text-indigo-600 font-bold">{test.examCode}</span></p>
          </div>
          <div className="flex gap-4">
            <div className="bg-slate-50 px-4 py-2 rounded-xl ring-1 ring-slate-200">
              <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Participants</p>
              <p className="text-2xl font-black text-slate-900">{participants.length}</p>
            </div>
            <div className="bg-indigo-50 px-4 py-2 rounded-xl ring-1 ring-indigo-100">
                ...
                <tr key={p._id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-bold text-slate-900">{p.person.fullName}</div>
                        <div className="text-xs text-slate-500">{p.person.email}</div>
                      </div>
                    </div>
                  </td>
        ...
        {/* Audit Modal/Overlay */}
        {selectedCandidate && (
          <Modal onClose={() => setSelectedCandidate(null)} className="bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-8 shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">Violation Audit</h2>
                  <p className="text-slate-500">Reviewing logs for {selectedCandidate.person.fullName}</p>
                </div>
