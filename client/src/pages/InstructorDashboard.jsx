      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Manage your tests and monitor candidate performance in real-time.</p>
          </div>
          <Link
            to="/create"
        ...
        {tests.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-dashed border-slate-300">
            ...
            <p className="mt-1 text-sm text-slate-500">Get started by creating your first exam code.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {tests.map((test) => (
              <div
                key={test.examCode}
                className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-all ring-1 ring-slate-200 overflow-hidden cursor-pointer group"
              ...
              >
                <div className="p-6">
                  ...
                  <h3 className="mt-4 text-xl font-bold text-slate-900 truncate">{test.title}</h3>
                  <div className="mt-2 flex items-center text-sm text-slate-500">
                    <code className="bg-slate-100 px-2 py-1 rounded text-indigo-600 font-mono text-xs">{test.examCode}</code>
                    <span className="mx-2 text-slate-300">•</span>
                    <span>{test.durationMinutes} mins</span>
                  </div>
                  
                  <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center text-indigo-600 font-semibold text-sm">
                    View Results
